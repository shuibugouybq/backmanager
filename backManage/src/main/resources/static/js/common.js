var $MB = (function () {
     ...
     * @param boolean autoInput    自动填充（有则true）
     * @param boolean startDate    起始日期
     * @return none
     */
    function _calenders(eles, dobubble, secondNot, autoInput, startDate) {
        ...
        $(eles).daterangepicker({
            ...
            "autoApply": true,
            "autoUpdateInput": autoInput || false,
            "locale": {
                "direction": "ltr",
                "format": formatDate,
                "separator": "~",
    ...
    function _setFormValues($form, data){
    	for (var p in data){
        	var $el = $form.find(":input[name='" + p + "']");
        	if(!$el.length>0){
        		$el = $form.find("select[name='" + p + "']");
        	}
        	$el.val(data[p]);
        } 
    }
    
    function _getDic(url){
    	var dictionaries;
    	$.ajax({
            type: "post",
            async: false,
    	...
    	return $.extend(constMap,dictionaries);
    }
    ...
    return {
        ...
        calenders: function (eles, dobubble, secondNot, autoInput, startDate) {
            _calenders(eles, dobubble, secondNot, autoInput, startDate);
        },
        setFormValues: function (id, data) {
            _setFormValues(id, data);
        },
        getDic: function (url) {
        	return _getDic(url);
        }
